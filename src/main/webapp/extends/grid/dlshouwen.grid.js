		...
		init : function(options, name) {
			...
			var gridObject = {
				...
				showProcessBar : function(){
					...
					**/
					/**不显示进度条 去掉样式bg-primary即可**/
					content += '<div class="dlshouwen-grid-process-bar-top" id="dlshouwen_grid_process_bar_top_'+gridReflectionObj.option.id+'"></div>';
					content += '<div class="dlshouwen-grid-process-bar-bottom" id="dlshouwen_grid_process_bar_bottom_'+gridReflectionObj.option.id+'"></div>';
...
//					});
				...
				editLoad : function(sparams){
					var obj = JSON.parse(sparams);
					//定义表格对象映像
					var gridReflectionObj = this;
					//设置初始化完成
					gridReflectionObj.init.toolsIsInit = true;
					gridReflectionObj.pager.pageSize = obj.pageSize;
					gridReflectionObj.pager.nowPage = obj.nowPage;
					gridReflectionObj.pager.recordCount = obj.recordCount;
					gridReflectionObj.pager.pageCount = obj.pageCount;
					gridReflectionObj.parameters = obj.parameters;
					gridReflectionObj.editRefresh(true);
				},
				//重新加载重载方法
				editRefresh : function(allReload){
					//定义表格对象映像
					var gridReflectionObj = this;
					gridReflectionObj.reload(allReload);
				},
