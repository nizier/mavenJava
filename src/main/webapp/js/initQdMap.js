...
var map = new AMap.Map("leafletmap", {resizeEnable: true,zoom: 18,center: [116.604005,40.079351]});
...
$("#checkin-island").click(function(){
	if($(this).is(":checked")){
	   showPartGisMarker(cameras, 0, 4);
	}else{
	   hidePartGisMarker(cameras, 0, 4);
	}
});

$("#security-checkpoint").click(function(){
	if($(this).is(":checked")){
	   showPartGisMarker(cameras, 4, 8);
	}else{
	   hidePartGisMarker(cameras, 4, 8);
	}
});

$("#boarding-gate").click(function(){
	if($(this).is(":checked")){
	   showPartGisMarker(cameras, 8, 12);
	}else{
	   hidePartGisMarker(cameras, 8, 12);
	}
});

$("#departure-hall").click(function(){
	if($(this).is(":checked")){
	   showPartGisMarker(cameras, 12, 16);
	}else{
	   hidePartGisMarker(cameras, 12, 16);
	}
});

$("#arrival-hall").click(function(){
	if($(this).is(":checked")){
	   showPartGisMarker(cameras, 16, 20);
	}else{
	   hidePartGisMarker(cameras, 16, 20);
	}
});

$("#baggage-trust").click(function(){
	if($(this).is(":checked")){
	   showPartGisMarker(cameras, 20, 24);
	}else{
	   hidePartGisMarker(cameras, 20, 24);
	}
});

$("#baggage-claim").click(function(){
	if($(this).is(":checked")){
	   showPartGisMarker(cameras, 24, 28);
	}else{
	   hidePartGisMarker(cameras, 24, 28);
	}
});

$("#lane-side").click(function(){
	if($(this).is(":checked")){
	   showPartGisMarker(cameras, 28, 32);
	}else{
	   hidePartGisMarker(cameras, 28, 32);
	}
});
...
$(document).ready(function(){
	...
	$("#passenger-distribution").click(function(){
	    if($(this).is(":checked")){
	        startHot();
	    }else{
...
function addEventMark(rootUrl){
	...
	for (var i = 0; i < eventLnglats.length; i++) {
		...
		html+='</div>';
		html+='<div class="layer-button">';
		//<a>录入核实信息</a><a>查看预案</a><a>误报</a>
  		html+='<a onclick="changeSp(\''+i+'\')">实时视频</a>';
  		html+='</div>';
...
function addEventMark1(rootUrl){
	...
	for (var i = 0; i < errorSxtlnglats.length; i++) {
		...
		html+='<div class="layer-button">';
		//<a>录入核实信息</a><a>误报</a>
  		html+='<a>查看预案</a><a onclick="changeSp(\''+i+'\')">实时视频</a>';
  		html+='</div>';
...
function addJyMark(rootUrl){
	...
	for (var i = 0; i < jylnglats.length; i++) {
	    marker = new AMap.Marker({
	        position: jylnglats[i],
	        map: map,
			icon:icon
	    });
...
function startHot(){
	setHot();
	hot = window.setInterval(function(){setHot();},3000);
}
...
$(document).ready(function(){
	...
	map.plugin(["AMap.MouseTool"], function() {
        ...
        AMap.event.addListener(mouseTool, "draw", function callback(e) {
            ...
            //openCameraInfo();
            showSelectedVideo("/sodb");
            //alert("end");
            //显示右侧视频
            $('#video_tab').show();
            $("#slide_img").attr('src','/sodb/img/GisLocation/right-arrow.png');
        });
       mouseTool.measureArea();  //调用鼠标工具的面积量测功能
    });

	/*AMap.event.addDomListener(document.getElementById('draw_point'), 'click', function() {
		 mouseTool.marker({
	...
	AMap.event.addDomListener(document.getElementById('draw_line'), 'click', function() {
	    mouseTool.polyline();
	}, false);*/
	AMap.event.addDomListener(document.getElementById('draw_polygon'), 'click', function() {
		mouseTool.polygon();
	}, false);
...
function openCameraInfo() {
	//构建信息窗体中显示的内容
	var info = [];
	info.push("<table id='camera_list'><tr><td id='camera_code'>摄像机编号<\/td><td>安装位置<\/td><td>状态<\/td><\/tr><tr><td>T2-F4-D2-10<\/td><td>T2航站楼4楼2入口<\/td><td class='success'>在线<\/td><\/tr><tr><td>T1-F1-D2-100<\/td><td>T1航站楼1楼2入口<\/td><td>离线<\/td><\/tr><tr><td>T2-F2-D3-99<\/td><td>T2航站楼2楼3入口<\/td><td>在线<\/td><\/tr><tr><td>T3-F3-D3-44<\/td><td>T3航站楼3楼3入口<\/td><td>在线<\/td><\/tr><tr><td>T1-F2-D2-10<\/td><td>T2航站楼2楼2入口<\/td><td>在线<\/td><\/tr><\/table>");
...
function showSelectedVideo(rootUrl){
	var path1 = rootUrl + '/img/GisLocation/slide_1.png';
	var path2 = rootUrl + '/img/GisLocation/slide_2.png';
	var path3 = rootUrl + '/img/GisLocation/slide_3.png';
	var path4 = rootUrl + '/img/GisLocation/slide_4.png';
	
	$("#slide_1").attr('src',path1);
	$("#slide_2").attr('src',path2);
	$("#slide_3").attr('src',path3);
	$("#slide_4").attr('src',path4);
}
